...
int my_gp_Read(graphP theGraph, std::list<std::vector<int> >& input, int n)
// input is an adjacency list
// example: first vector is for vertex 0:
// it may contain 1, 2, 3, 5
{ 

int N=n, I, J, ErrorCode;

     if (gp_InitGraph(theGraph, N) != OK)
          return NOTOK;

     std::list<std::vector<int> >::iterator it = input.begin();
     for (I = 0, ErrorCode = OK; I < N && ErrorCode==OK; I++, it++)
     {
          theGraph->G[I].v = I;
          for (auto& J: *it) 
          { 
...
int  my_gp_Write(graphP theGraph, std::vector<std::vector<int> >& output)
// output is empty
// it will be an adjacency list after writing
// the i-th vector is the adjacency list of the (i+1)-th vertexx 
// example: the first vector only contains the neighbors of vertex 0: 1, 2, 5, 8
{
int I, J;
